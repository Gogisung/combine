  ...
  >
    ...
    <v-col cols="12" md="6">
      <v-btn
        ...
        color="success"
        class="mr-4"
        @click="validate({subsidiaryId, common, title, htmlForEditor, })"
      >
        등록하기
      </v-btn>
    </v-col>
  ...
  data() {
    return {
      handleImageAdded: function(file, Editor, cursorLocation, resetUploader) {
        ...
        axios.post("http://localhost:3065/api/post/images", formData, {
          withCredentials: true,
        }).then((res) => {
          // let url = res.config.url; // Get url from response
          let url = `http://localhost:3065/${res.data}`; // Get url from response
